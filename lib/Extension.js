...
exports = module.exports = function Extension(schema, options) {
    ...
    //API Keys
    var API_KEY = upperTableName+'ApiKey';
    var API_KEY_CREATE_METHOD = 'create' + API_KEY;
    var API_KEY_FIND_METHOD = 'find' + API_KEY;
//    var API_KEY_REMOVE_METHOD = 'remove' + API_KEY;
    var API_KEY_FIND_BY_METHOD = 'findBy' + API_KEY;
    //API Secrets
    var API_SECRET = upperTableName+'ApiSecret';
    var API_SECRET_CREATE_METHOD = 'create' + API_SECRET;
    var API_SECRET_FIND_METHOD = 'find' + API_SECRET;
//    var API_SECRET_REMOVE_METHOD = 'remove' + API_SECRET;
    var API_SECRET_FIND_BY_METHOD = 'findBy' + API_SECRET;
     ...
     * @param next
     */
    schema.statics[API_KEY_FIND_BY_METHOD] = function(apiKey, next){
        var self = this;
     ...
     * @param next
     */
    schema.statics[API_SECRET_FIND_BY_METHOD] = function(apiSecret, next){
        var self = this;
        ...
        if (!extension) {
            ...
            extensionSchema.statics.createExtension = function (model, options, next) {
                ...
                this.create(options, function (err, result) {
                    ...
                    if(result){
                        result[API_KEY_CREATE_METHOD](function(err){
                            if(err){
                                log.error('Error creating apiKey', err);
                                next(err);
                            }else{
                                result[API_SECRET_CREATE_METHOD](function(err){
                                    if(err){
                                        log.error('Error creating apiSecret', err);
                                        next(err);
                                    }else{
                                        result.save(next);
                                    }
            ...
            extensionSchema.statics.findByApiKey = function(token, next){
                this[API_KEY_FIND_BY_METHOD](token, next);
            };
            ...
            extensionSchema.statics.findByApiSecret = function(token, next){
                this[API_SECRET_FIND_BY_METHOD](token, next);
            };
            ...
            /**
             * Generate a new Application API Secret
             * @param next
             */
            extensionSchema.methods.createApiSecret = function(next){
                this[API_SECRET_CREATE_METHOD](next);
            };
            ...
            extensionSchema.methods.apiKey = function(next){
                this[API_KEY_FIND_METHOD](next);
            };

            /**
             * Retrieve the Applications API Secret
             * @param next
             */
            extensionSchema.methods.apiSecret = function(next){
                this[API_SECRET_FIND_METHOD](next);
            };

            extensionSchema.plugin(Token.plugin, {tableName:upperTableName+'ApiKey'});
            extensionSchema.plugin(Token.plugin, {tableName:upperTableName+'ApiSecret'});
